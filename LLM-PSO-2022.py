...
def HighLevel(current_operators):
    ...
    Target: Early={1 if phase == "AGGRESSIVE_EXPLORATION" else 0}
    ...
    try:
        response = client.chat.completions.create(
            ...
            messages=[
                {"role": "system",
                 "content": f"""You are an optimization expert. Generate the BEST sequence of {PopSize} integers (0-9) for superior performance on ALL benchmark functions.
                 Key principles:
...
def main(dim):
    ...
    CEC2022 = [F112022(dim),F122022(dim)]
    ...
    for i in range(len(CEC2022)):
        ...
        # 3. 计算每个算法10次实验的平均轨迹（仅替换shade为wde）
        ...
        # 4. 组织平均结果（仅替换shade为wde）
        average_results = np.vstack([
            llm_pso_avg,
            pso_avg,
            ppso_avg,
            sade_avg,
            wde_avg,  # 替换shade_avg为wde_avg
            de_avg,
            dms_pso_avg,
            clpso_avg,
            pso_ga_avg
        ])  # shape: (9, max_len)，9行（算法），每行max_len列（迭代步）

        # 5. 保存平均值结果（原代码不变）
        save_path = f"./LLMPSO_Data/CEC2022_20D/F{i + 11}_{dim}D_Average.csv"
        np.savetxt(save_path, average_results, delimiter=",", fmt="%.6f")  # 不添加.T

        print(f"已保存F{i + 1}_{dim}D 10次实验平均值数据，形状: {average_results.shape}")
        print(f"平均值数据维度说明：行=算法数(9)，列 =迭代步数({average_results.shape[1]})")

if __name__ == "__main__":
    if os.path.exists('LLMPSO_Data/CEC2022_20D') == False:
        os.makedirs('LLMPSO_Data/CEC2022_20D')
    Dims = [20]
    for dim in Dims:
        main(dim)
